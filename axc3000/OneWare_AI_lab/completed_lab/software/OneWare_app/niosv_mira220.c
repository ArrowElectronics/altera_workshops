...
* - Added support for reading Pixel Reader registers (Frame Counter, Column, Row)
* - New commands: P (full status), F (frame counter), C (column/row)
* - Pixel Reader Base Address: 0x00070000
*
*/
...
// DPHY 0
#define D0_CAP                  (0x03 * 4)
#define DPHY_CSR                (0x10 * 4)
#define CLK_CSR                 (0x1C * 4)
...
#define ONE_AI_BASE                 ONE_AI_INTERFACE_0_BASE  // Use system.h definition
...
// 512x512_30fps_12b_2lanes_1500MHz
static const struct mira220_reg full_512x512_30fps_12b_2lanes_reg[] = {
		...
		{0x1012,0x48},  //Readout && Exposure.VBLANK_A(0)
		...
		{0x102B,0x5E},  //Readout && Exposure.ROW_LENGTH_A(0)
		...
		{0x107D,0xBC},  //Vertical ROI.VSTART0_A(0)
		{0x107E,0x1},  //Vertical ROI.VSTART0_A(1)
		{0x1087,0x0},  //Vertical ROI.VSIZE0_A(0)
		...
		{0x107D,0xBC},  //Vertical ROI.VSTART1_A(0)
		{0x107E,0x1},  //Vertical ROI.VSTART1_A(1)
		{0x107F,0x0},  //Vertical ROI.VSTART1_A(2)
		{0x1087,0x0},  //Vertical ROI.VSIZE1_A(0)
		...
		{0x107D,0xBC},  //Vertical ROI.VSTART2_A(0)
		...
		{0x1087,0x0},  //Vertical ROI.VSIZE2_A(0)
		...
		{0x24DC,0x13},  //Defect Pixel Correction.DC_ENABLE(0)
		{0x24DC,0x11},  //Defect Pixel Correction.DC_MODE(0)
		{0x24DC,0x1},  //Defect Pixel Correction.DC_REPLACEMENT_VALUE(0)
		...
		{0x10D7,0x0},  //Illumination Trigger.ILLUM_EN(0)
...
int main()
{
    ...
    // Camera in Reset
    IOWR_ALTERA_AVALON_PIO_DATA(MIPI_PIO_BASE, MIRA220_RESETn);
    ...
    tempo = IORD_ALTERA_AVALON_SYSID_QSYS_ID(SYS_ID_BASE);
    alt_putstr("System ID: 0x");
    word2ascii(tempo >> 16);
    alt_putchar('-');
    alt_putchar(HEX2ASCII((tempo >> 12) & 0x000F));
    alt_putchar(HEX2ASCII((tempo >> 8)  & 0x000F));
    alt_putchar('.');
    alt_putchar(HEX2ASCII((tempo >> 4)  & 0x000F));
    alt_putchar('.');
    alt_putchar(HEX2ASCII(tempo & 0x000F));
    alt_putstr("\r\n\n");
	...
	// activate sign of life in FPGA
    IOWR_ALTERA_AVALON_PIO_DATA(RGB_PIO_BASE, 0x05);  // Only Green LED on
    ...
    while(1) {
        ...
        case 'i' :
            ...
            alt_putstr("Exposure Index is : ");
            byte2bcd(exp_time_index);
            alt_putstr("\r\n");
            break;
        ...
        case 'd' :
              if (exp_time_index != 0) {
               ...
               alt_putstr("\r\n");
            break;
        case 'P' : // Display AI Interface Status
        case 'p' :
            display_ai_status();
            break;
        case 'C' : // Display CNN Addressing Methods Test
        case 'c' :
            test_cnn_addressing_methods();
            break;
        case 'S' : // System Status Check
        case 's' :
            {
                alt_putstr("=== System Status Check ===\r\n");
                
                // Check camera chip version
                peek(MIRA220_REG_CHIP_VER, 1);
                alt_putstr("Chip Version: 0x");
                alt_putchar(HEX2ASCII((rData[0] >> 4) & 0x0F));
                alt_putchar(HEX2ASCII(rData[0] & 0x0F));
                alt_putstr("\r\n");
                
                // Check imager state
                peek(MIRA220_IMAGER_STATE_REG, 1);
                alt_putstr("Imager State: 0x");
                alt_putchar(HEX2ASCII((rData[0] >> 4) & 0x0F));
                alt_putchar(HEX2ASCII(rData[0] & 0x0F));
                alt_putstr("\r\n");
                
                // Check imager run status
                peek(MIRA220_IMAGER_RUN_REG, 1);
                alt_putstr("Imager Run: 0x");
                alt_putchar(HEX2ASCII((rData[0] >> 4) & 0x0F));
                alt_putchar(HEX2ASCII(rData[0] & 0x0F));
                alt_putstr(" (should be 01)\r\n");
                
                // Check VFW status
                alt_u32 vfw_status = IORD_32DIRECT(MIPI_SYSTEM_0_VVP_VFW_BASE, VFW_IRQ_STATUS);
                alt_putstr("VFW IRQ Status: 0x");
                word2ascii((vfw_status >> 16) & 0xFFFF);
                word2ascii(vfw_status & 0xFFFF);
                alt_putstr("\r\n");
                
                // Check VFW run status
                alt_u32 vfw_run = IORD_32DIRECT(MIPI_SYSTEM_0_VVP_VFW_BASE, VFW_RUN);
                alt_putstr("VFW Run: 0x");
                word2ascii((vfw_run >> 16) & 0xFFFF);
                word2ascii(vfw_run & 0xFFFF);
                alt_putstr(" (should be 01)\r\n");
            ...
            break;
   	default :
			// Ignore unknown characters (whitespace, control chars, etc.)
			// Only show menu for printable characters
			if (uart_rx_char >= 32 && uart_rx_char <= 126) {
				signOn();
			}
    		break;
...
void signOn ()
{
    ...
    alt_putstr("  T -- MIRA220 Test Pattern Enable\r\n");
    alt_putstr("  U -- MIRA220 Test Pattern Disable (default)\r\n");
    alt_putstr("  I -- Increment Exposure\r\n");
    alt_putstr("  D -- Decrement Exposure\r\n");
    alt_putstr("  P -- Display Pixel Reader Status (Frame, Column, Row)\r\n");
    alt_putstr("  F -- Display Frame Counter only\r\n");
    alt_putstr("  C -- Display Current Column/Row Position\r\n");
    alt_putstr("  X -- Debug: Test Register 3 (should be 18)\r\n");
    alt_putstr("  S -- System Status Check (Camera & VFW)\r\n");
    alt_putstr(">");
}
...
alt_u8 init_MIRA220 () {
    ...
    // In LITE mode
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_DEMOSAIC_BASE,DEMOSAIC_IMG_WIDTH,512);
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_DEMOSAIC_BASE,DEMOSAIC_IMG_HEIGHT,512);
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_DEMOSAIC_BASE,DEMOSAIC_CONTROL,0x04);	// Mira2202 Color Filter BG/GR
	...
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_WBC_BASE,WBC_IMG_WIDTH,512);				// IMG_INFO_WIDTH
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_WBC_BASE,WBC_IMG_HEIGHT,512);				// IMG_INFO_HEIGHT

	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_WBC_BASE,WBC_CFA_00_COLOR_SCALER,(2048 + 1536));	// CFA_00_COLOR_SCALER
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_WBC_BASE,WBC_CFA_01_COLOR_SCALER,(2048 + 4096));	// CFA_01_COLOR_SCALER
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_WBC_BASE,WBC_CFA_10_COLOR_SCALER,(2048 + 4096));	// CFA_10_COLOR_SCALER
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_WBC_BASE,WBC_CFA_11_COLOR_SCALER,(2048 + 3072));	// CFA_11_COLOR_SCALER
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_WBC_BASE,WBC_COMMIT,0x01);		// COMMIT

	// Initialize the Scaler
    // In LITE mode
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_SCALER_BASE,0x120,512);	// IMG_INFO_WIDTH
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_SCALER_BASE,0x124,512);	// IMG_INFO_HEIGHT
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_SCALER_BASE,0x148,128);	// OUTPUT_WIDTH
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_SCALER_BASE,0x14C,128);	// OUTPUT_HEIGHT

	//alt_putstr("Initializing Video Frame Writer\r\n");
	// Write image dimensions
	IOWR_32DIRECT (MIPI_SYSTEM_0_VVP_VFW_BASE,VVP_VFW_IMG_WIDTH, (128*3));	// IMG_INFO_WIDTH
	IOWR_32DIRECT (MIPI_SYSTEM_0_VVP_VFW_BASE,VVP_VFW_IMG_HEIGHT, 128);		// IMG_INFO_HEIGHT
	IOWR_32DIRECT (MIPI_SYSTEM_0_VVP_VFW_BASE,VVP_VFW_NUM_BUFFERS, 0x02);	// Num Buffers
	IOWR_32DIRECT (MIPI_SYSTEM_0_VVP_VFW_BASE,VVP_VFW_BUFFER_BASE, 0x80000);	// BUFFER_BASE
	IOWR_32DIRECT (MIPI_SYSTEM_0_VVP_VFW_BASE,VVP_VFW_INTER_BUFFER_OFFSET, 50000);	// BUFFER SPACING
	IOWR_32DIRECT (MIPI_SYSTEM_0_VVP_VFW_BASE,VVP_VFW_INTER_LINE_OFFSET, (128*3));	// LINE SPACING
    ...
    // Camera On
    IOWR_ALTERA_AVALON_PIO_DATA(MIPI_PIO_BASE, MIRA220_ENABLE);
    usleep (500000);
    usleep (500000);
	...
	for (int i = 0; i < sizeof(full_512x512_30fps_12b_2lanes_reg)/sizeof(struct mira220_reg); i++)
	{
		poke(full_512x512_30fps_12b_2lanes_reg[i].address, full_512x512_30fps_12b_2lanes_reg[i].val, 1);
		//alt_putchar('.');
	}
...
void VFW_isr (void* context)
{
    ...
    IOWR_ALTERA_AVALON_PIO_DATA(RGB_PIO_BASE, 0x06);  // only B on
	//alt_putstr("In VFW IRQ\r\n");

    // Acknowledge the Buffer. No need for COMMIT
	IOWR_32DIRECT(MIPI_SYSTEM_0_VVP_VFW_BASE,VFW_BUFFER_ACKNOWLEDGE,0x00);	// BUFFER_ACKNOWLEDGE    // Clear the interrupt bit
...
alt_u32 read_ai_register(alt_u8 reg_addr)
{
    ...
    reg_value = IORD_32DIRECT(ONE_AI_BASE, reg_addr * 4);
...
void test_cnn_addressing_methods(void)
{
    ...
    for(i = 0; i < 10; i++)
    {
        ...
        IOWR_32DIRECT(ONE_AI_BASE, ONE_AI_USER_OBJECTS_CLASSES * 4, (i << 16) | 0);
